...
- **Sentenza preprocessing pitfall**: `sentenza::split_sentences` preprocesses text before splitting: it collapses `\s{2,}` to a single space, collapses comma runs, and normalizes dash runs (`--`, `---`, mixed `-`/em-dash) to em dashes. Returned sentence strings won't match the original text verbatim when the source has double spaces (common in LaTeX/PDF paste) or dash/comma runs, and HTML comments embedded in a paragraph come back mangled beyond recognition. The scope resolver therefore (1) blanks the whole document length-preservingly via `blank_comments()` before paragraph splitting and sentence splitting, driven by `scanner::comment_byte_ranges()` so blanking covers exactly what the renderer treats as annotations (standard `<!-- -->` comments, fenced-code openers, and unterminated `<!---` are left intact), (2) locates sentences back in the original paragraph with `ws_flexible_find()`, a matcher that reverses each round-trippable normalization (whitespace runs, comma runs, dash runs; a lone `-` never matches an em dash), (3) fails resolution outright when a sentence still cannot be located (non-round-trippable normalizations such as German quote rewriting or CJK symbol stripping): no highlight beats a wrong highlight, and (4) walks backward only past paragraphs that are truly empty after blanking (comment-only paragraphs); forward resolution never crosses a `\n\n`. Any new code that matches sentenza output against original text must account for these mismatches.
