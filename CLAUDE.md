...
- **Sentenza preprocessing pitfall**: `sentenza::split_sentences` preprocesses text before splitting: it collapses `\s{2,}` to a single space, collapses comma runs, and normalizes dash runs (`--`, `---`) to em dashes. Returned sentence strings won't match the original text verbatim when the source has double spaces (common in LaTeX/PDF paste), and HTML comments embedded in a paragraph come back mangled beyond recognition. The scope resolver therefore (1) blanks `<!--- --->` annotation comments length-preservingly via `blank_comments()` before splitting (standard `<!-- -->` comments are not blanked), (2) uses `ws_flexible_find()` — a whitespace-tolerant search — instead of exact `str::find()` to locate sentences back in the original paragraph, (3) skips unlocatable sentences instead of failing the whole paragraph, and (4) walks backward past paragraphs that yield no locatable sentences (e.g. comment-only paragraphs); forward resolution never crosses a `\n\n`. Any new code that matches sentenza output against original text must account for these mismatches.
